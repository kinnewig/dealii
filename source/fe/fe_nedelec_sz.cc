...
template <>
void
FE_NedelecSZ<3, 3>::precompute_line_to_cell_map(
  const DoFHandler<3, 3> &dof_handler)
{
  ...
  line_to_cell_map = std::vector<std::vector<
    std::set<typename Triangulation<dim, spacedim>::active_cell_iterator>>>(
      ...
      for (unsigned int line : cell->line_indices())
        {
          ...
          std::set_intersection(
            ...
            std::inserter(
              line_to_cell_map[cell->active_cell_index()][line],
              line_to_cell_map[cell->active_cell_index()][line].begin()));
        }
    }
}
...
template <int dim, int spacedim>
void
FE_NedelecSZ<dim, spacedim>::precompute_line_to_cell_map(
          ...
          if (flags & (update_values | update_gradients | update_hessians))
            {
              ...
              if (line_to_cell_map.size() != 0)
                for (unsigned int line : cell->line_indices())
                  {
                    // if we already now, this is a hanging edge, we do not
                    // need to run further checks wether this is a hanging edge
                    // or not
              ...
              for (unsigned int line : cell->line_indices())
                {
                  ...
                  // get the direction in comparision to the reference element
                  if (v0_glob > v1_glob)
                    {
                      // Opposite to global numbering on our reference element
                      edge_sign[line] = -1.0;
