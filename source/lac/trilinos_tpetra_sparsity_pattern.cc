  ...
  namespace TpetraWrappers
  {
    namespace SparsityPatternIterators
    {
      ...
      Accessor::visit_present_row()
      {
        ...
        if (static_cast<unsigned long>(this->a_row) ==
        ...
        colnum_cache =
          Teuchos::RCP<std::vector<dealii::types::signed_global_dof_index>>(
            new std::vector<dealii::types::signed_global_dof_index>(
    ...
    {
      column_space_map =
        Teuchos::rcp(new MapType(TrilinosWrappers::types::int_type(0),
                                 TrilinosWrappers::types::int_type(0),
                                 Utilities::Trilinos::tpetra_comm_self()));
      graph =
        Teuchos::rcp(new GraphType(column_space_map, column_space_map, 0));
      graph->fillComplete();
    }
    ...
    SparsityPattern::SparsityPattern(const SparsityPattern &input_sparsity)
      ...
      , graph(new GraphType(column_space_map, column_space_map, 0))
    {
      (void)input_sparsity;
    ...
    namespace
    {
      ...
      void
      reinit_sp(const Teuchos::RCP<MapType> &row_map,
                ...
                Teuchos::RCP<GraphType>     &nonlocal_graph)
      {
        ...
        // initialization.
        graph =
          Teuchos::rcp(new GraphType(row_map, row_map, n_entries_per_row));
      }
                ...
                Teuchos::RCP<GraphType>      &nonlocal_graph)
      {
        ...
        graph =
          Teuchos::rcp(new GraphType(row_map, col_map, local_entries_per_row));
      }
                ...
                Teuchos::RCP<GraphType>     &nonlocal_graph)
      {
        ...
        if (row_map->getComm()->getSize() > 1)
          graph = Teuchos::rcp(new GraphType(
            row_map,
            Teuchos::ArrayView<const unsigned long int>(
        ...
        if (exchange_data == false)
          ...
          for (unsigned long row = 0; row < sp.n_rows(); ++row)
            {
              const TrilinosWrappers::types::int_type row_length =
                sp.row_length(row);
              if (row_length == 0)
...
#  endif
    ...
    {
      SparsityPatternBase::resize(sp.n_rows(), sp.n_cols());
      auto rows = Teuchos::rcp(
        new Tpetra::Map<int, dealii::types::signed_global_dof_index>(
          TrilinosWrappers::types::int_type(sp.n_rows()),
          0,
          Utilities::Trilinos::tpetra_comm_self()));
      auto columns = Teuchos::rcp(
        new Tpetra::Map<int, dealii::types::signed_global_dof_index>(
      ...
      /*try
      ...
      catch (const int error_code)
        {
          AssertThrow(
            false,
            ExcMessage(
              "The Tpetra::CrsGraph<int,
      dealii::types::signed_global_dof_index>::OptimizeStorage() function " "has
      thrown an error with code " + std::to_string(error_code) +
    ...
    SparsityPattern::exists(const size_type i, const size_type j) const
    {
      ...
      return static_cast<unsigned long>(local_col_index) != col_indices.size();
    }
    ...
    void
    SparsityPattern::add_row_entries(
